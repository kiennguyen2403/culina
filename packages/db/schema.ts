...
export const interests = mysqlTable("Interest", {
  id: int(`id`).autoincrement().notNull().primaryKey(),
  name: varchar(`name`, { length: 191 }).notNull(),
});

export const userInterests = mysqlTable("UserInterest", {
  id: int(`id`).autoincrement().notNull().primaryKey(),
  user_id: varchar(`user_id`, { length: 191 }).notNull(),
  interest_id: int(`interest_id`).notNull(),
});

export const recipes = mysqlTable("Recipe", {
  recipe_id: int(`recipe_id`).autoincrement().notNull().primaryKey(),
  user_id: varchar(`user_id`, { length: 191 }).notNull(),
  name: varchar(`name`, { length: 191 }).notNull(),
  description: varchar(`description`, { length: 191 }).notNull(),
  image: varchar(`image`, { length: 191 }).notNull(),
  ingredients: varchar(`ingredients`, { length: 191 }).notNull(),
  instructions: varchar(`instructions`, { length: 191 }).notNull(),
  prep_time: int(`prep_time`).notNull(),
  cook_time: int(`cook_time`).notNull(),
...
export const post = mysqlTable(
  "Post",
  {
    post_id: serial("post_id").autoincrement().notNull().primaryKey(),
    user_id: varchar("user_id", { length: 191 }).notNull(),
    content: text("content").notNull(),
    post_likes: int("post_id"),
    // Add other fields for post details like title, description, etc.
  },
  (table) => ({
    post_key: uniqueIndex(`post_key`).on(table.post_id),
  })
);

export const postPicture = mysqlTable("PostPictures", {
  picture_id: int("picture_id").autoincrement().notNull().primaryKey(),
  post_id: int("post_id").notNull(),
  picture_path: varchar("picture_path", { length: 255 }).notNull(),
});

export const postComment = mysqlTable("PostComments", {
  comment_id: int("comment_id").autoincrement().notNull().primaryKey(),
  post_id: int("post_id").notNull(),
  user_id: varchar("user_id", { length: 191 }).notNull(),
...
export type UserInterest = InferModel<typeof userInterests>;
...
export type PostPicture = InferModel<typeof postPicture>;
export type PostComment = InferModel<typeof postComment>;
